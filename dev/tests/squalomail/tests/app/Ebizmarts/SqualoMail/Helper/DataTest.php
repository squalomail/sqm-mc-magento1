...
class Ebizmarts_SqualoMail_Helper_DataTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testIsCheckoutSubscribeEnabled()
    {
        ...
        $helperMock->expects($this->once())->method('getCheckoutSubscribeValue')->with($scopeId, $scope)
            ->willReturn(Ebizmarts_SqualoMail_Model_System_Config_Source_Checkoutsubscribe::NOT_CHECKED_BY_DEFAULT);
    ...
    public function testAddResendFilter()
    {
        ...
        $helperMock->addResendFilter($collectionMock, $storeId, Ebizmarts_SqualoMail_Model_Config::IS_ORDER);
    }
    ...
    public function testDeleteAllConfiguredSQMStoreLocalData()
    {
        ...
        $where = "status = 'pending' AND store_id = $squalomailStoreId";
        $tableName = 'squalomail_sync_batches';
        ...
        $helperMock = $this->getMockBuilder(Ebizmarts_SqualoMail_Helper_Data::class)
            ->disableOriginalConstructor()
            ->setMethods(array('getConfig', 'getCoreResource', 'saveSqualomailConfig', 'getAllStoresForScope'))
            ->getMock();
        ...
        $params ['param9'] = array(Ebizmarts_SqualoMail_Model_Config::ECOMMERCE_CART_LAST_ID, $scope, $scopeId);
        $params ['param10'] = array(Ebizmarts_SqualoMail_Model_Config::ECOMMERCE_PCD_LAST_ID, $scope, $scopeId);
        $params ['param11'] = array(Ebizmarts_SqualoMail_Model_Config::ECOMMERCE_RESEND_ENABLED, $scope, $scopeId);
        $params ['param12'] = array(Ebizmarts_SqualoMail_Model_Config::ECOMMERCE_RESEND_TURN, $scope, $scopeId);
        ...
        $dbAdapterInterfaceMock
            ->expects($this->once())
            ->method('quoteInto')
            ->with("status = 'pending' AND store_id = ?", $squalomailStoreId)
            ->willReturn($where);
        $dbAdapterInterfaceMock
            ->expects($this->once())
            ->method('update')
    ...
    public function testGetDateSyncFinishByStoreId()
    {
        ...
        $helperMock
            ...
            ->with(Ebizmarts_SqualoMail_Model_Config::ECOMMERCE_SYNC_DATE . "_$squalomailStoreId", $scopeId, $scope);
    ...
    public function testSaveSqualoMailConfig()
    {
        $helperMock = $this->getMockBuilder(Ebizmarts_SqualoMail_Helper_Data::class)
            ->disableOriginalConstructor()
        ...
        $configMock
            ->expects($this->once())
            ->method('saveConfig')
            ->with(Ebizmarts_SqualoMail_Model_Config::GENERAL_MIGRATE_FROM_116, 1, 'default', 0);
        $configMock->expects($this->once())->method('cleanCache');
        ...
        return array(
            array(
                array(
                    ...
                    'configImageSize' => Ebizmarts_SqualoMail_Helper_Data::DEFAULT_SIZE
                )
            ),
            array(
                array(
                    'imageSize' => Ebizmarts_SqualoMail_Model_Config::IMAGE_SIZE_SMALL,
                    'configImageSize' => Ebizmarts_SqualoMail_Helper_Data::SMALL_SIZE
                )
            ),
            array(
                array(
                    'imageSize' => Ebizmarts_SqualoMail_Model_Config::IMAGE_SIZE_THUMBNAIL,
                    'configImageSize' => Ebizmarts_SqualoMail_Helper_Data::THUMBNAIL_SIZE
                )
            )
        );
    }
    ...
    public function testGetOriginalImageUrlById()
    {
        ...
        $imageSize = Ebizmarts_SqualoMail_Model_Config::IMAGE_SIZE_DEFAULT;
        $imageUrl = 'http://magento.com/catalog/product/image.jpg';
        $configImageSize = Ebizmarts_SqualoMail_Helper_Data::ORIGINAL_SIZE;
    ...
    public function testRemoveEcommerceSyncDataDeleteAll()
    {
        $scopeId = 0;
        $scope = 'default';
        $filters = Ebizmarts_SqualoMail_Model_Config::IS_ORDER;
        $deleteErrorsOnly = false;

        $connectionType = 'core_write';
        $squalomailEcommTableAlias = 'squalomail/ecommercesyncdata';
        $squalomailEcommTableName = 'squalomail_ecommerce_sync_data';

        $squalomailStoreId = 'a1s2d3f4g5h6j7k8l9n0';
        $storeReturnWhere = "squalomail_store_id = " . $squalomailStoreId;
        $storeWhere = "squalomail_store_id = ?";

        $squalomailFilters = Ebizmarts_SqualoMail_Model_Config::IS_ORDER;
        $filterReturnWhere = "type IN (" . $squalomailFilters . ')';
        $filterWhere = "type IN (?)";
        ...
        $coreResourceMock
            ->expects($this->once())
            ->method('getTableName')
            ->with($squalomailEcommTableAlias)
            ->willReturn($squalomailEcommTableName);

        $dbAdapterInterfaceMock->expects($this->exactly(2))->method('quoteInto')
            ->withConsecutive(array($storeWhere, $squalomailStoreId), array($filterWhere, $squalomailFilters))
            ->willReturnOnConsecutiveCalls($storeReturnWhere, $filterReturnWhere);

        $dbAdapterInterfaceMock->expects($this->once())->method('delete')
            ->with($squalomailEcommTableName, array($storeReturnWhere, $filterReturnWhere));
    ...
    public function testRemoveAllEcommerceSynddataErrors()
    {
        ...
        $tableAlias = 'squalomail/ecommercesyncdata';
        $errorSyncWhere = "squalomail_sync_error != ''";
        ...
        $dbAdapterInterfaceMock
            ->expects($this->once())
            ->method('quoteInto')
            ->with($filterWhere, $squalomailFilters)
    ...
    public function testRemoveEcommerceSyncDataBySQMStore()
    {
        ...
        $tableName = 'squalomail_ecommerce_sync_data';
        $tableAlias = 'squalomail/ecommercesyncdata';
        $storeReturnWhere = "squalomail_store_id = $squalomailStoreId AND squalomail_sync_error != ''";
        ...
        $dbAdapterInterfaceMock->expects($this->exactly(2))->method('quoteInto')
            ->withConsecutive(
                array(
                    "squalomail_store_id = ? AND squalomail_sync_error != ''", $squalomailStoreId
                ),
                array(
                    $filterWhere, $squalomailFilters
                )
            )
            ->willReturnOnConsecutiveCalls($storeReturnWhere, $filterReturnWhere);
        $dbAdapterInterfaceMock
    ...
    public function testClearErrorGridExcludeSubscribers()
    {
        ...
        $connectionType = 'core_write';
        $squalomailEcommTableAlias = 'squalomail/squalomailerrors';
        $squalomailEcommTableName = 'squalomail_errors';
        ...
        $squalomailFilters = Ebizmarts_SqualoMail_Model_Config::IS_ORDER;
        $filterReturnWhere = "regtype IN (" . $squalomailFilters . ')';
        ...
        $helperMock->clearErrorGrid($scopeId, $scope, $excludeSubscribers, $squalomailFilters);
    }
    ...
    public function testClearErrorGridIncludeSubscribersForDefaultScope()
    {
        ...
        $dbAdapterInterfaceMock
            ->expects($this->once())
            ->method('delete')
            ->with($squalomailEcommTableName, array($filterReturnWhere));
    ...
    public function testClearErrorGridIncludeSubscribersForStoreView()
    {
        ...
        $dbAdapterInterfaceMock
            ->expects($this->once())
            ->method('delete')
            ->with($squalomailEcommTableName, array($storeReturnWhere));
    ...
    public function testIsNewApiKeyForSameAccount()
    {
        ...
        $apiRootMock = $this->getMockBuilder(SqualoMail_Root::class)
            ->disableOriginalConstructor()
    ...
    public function testResendSubscribers()
    {
        ...
        $setCondition = array('squalomail_sync_delta' => '0000-00-00 00:00:00', 'squalomail_sync_error' => '');
    ...
    public function testSaveLastItemsSent()
    {
        ...
        $filters = array(
            ...
            Ebizmarts_SqualoMail_Model_Config::IS_PROMO_CODE . ', ' . Ebizmarts_SqualoMail_Model_Config::IS_PROMO_RULE);
        ...
        $promoCodeLastId = 10;
        $configValues = array(
            array(Ebizmarts_SqualoMail_Model_Config::ECOMMERCE_CUSTOMER_LAST_ID, $customerLastId),
            array(Ebizmarts_SqualoMail_Model_Config::ECOMMERCE_PRODUCT_LAST_ID, $productLastId),
            array(Ebizmarts_SqualoMail_Model_Config::ECOMMERCE_ORDER_LAST_ID, $orderLastId),
            array(Ebizmarts_SqualoMail_Model_Config::ECOMMERCE_CART_LAST_ID, $cartLastId),
            array(Ebizmarts_SqualoMail_Model_Config::ECOMMERCE_PCD_LAST_ID, $promoCodeLastId),
            array(Ebizmarts_SqualoMail_Model_Config::ECOMMERCE_RESEND_ENABLED, $resendEnabled),
            array(Ebizmarts_SqualoMail_Model_Config::ECOMMERCE_RESEND_TURN, $resendTurn)
        );
        ...
        $helperMock->expects($this->once())->method('saveSqualomailConfig')->with($configValues, $scopeId, $scope);
    ...
    public function testGetListInterestCategoriesByKeyAndList()
    {
        ...
        $apiListsMock = $this->getMockBuilder(Ebizmarts_SqualoMail::class)
            ->disableOriginalConstructor()
            ->setMethods(array('getInterestCategory'))
            ->getMock();

        $apiListsInterestCategoryMock = $this->getMockBuilder(SqualoMail_ListsInterestCategory::class)
            ->disableOriginalConstructor()
    ...
    public function testGetListInterestGroups()
    {
        ...
        $apiListsInterestCategoryInterestsMock = $this->getMockBuilder(SqualoMail_ListInterestCategoryInterests::class)
            ->disableOriginalConstructor()
            ->setMethods(array('getAll'))
            ->getMock();
    ...
    public function testGetInterest()
    {
        ...
        $apiListsInterestCategoryInterestsMock = $this->getMockBuilder(
            SqualoMail_ListInterestCategoryInterests::class
        )
            ->disableOriginalConstructor()
            ->setMethods(array('getAll'))
            ->getMock();
    ...
    {
        $storeId = 1;
        $jsUrl = 'https://api.staging.squalomail.com/mc/v3/ecommerce/stores/sitejs';
        $squalomailStoreId = 'a1s2d3f4g5h6j7k8l9p0';
    ...
    public function testCreateMergeFields()
    {
        ...
        $scope = 'stores';
        $listId = "b514eebd1a";
        $mapFieldsSerialized = 'a:10:{s:18:"_1468601283719_719";a:2:{s:9:"squalomail";s:7:"WEBSITE";' .
            's:7:"magento";s:1:"1";}s:18:"_1468609069544_544";a:2:{s:9:"squalomail";s:7:"STOREID";' .
            's:7:"magento";s:1:"2";}s:18:"_1469026825907_907";a:2:{s:9:"squalomail";s:9:"STORENAME";' .
            's:7:"magento";s:1:"3";}s:18:"_1469027411717_717";a:2:{s:9:"squalomail";s:6:"PREFIX";' .
            's:7:"magento";s:1:"4";}s:18:"_1469027418285_285";a:2:{s:9:"squalomail";s:5:"FNAME";' .
            's:7:"magento";s:1:"5";}}';

        $mapFieldsUnserialized = array(
            "_1468601283719_719" => array("squalomail" => "WEBSITE", "magento" => "1"),
            "_1468609069544_544" => array("squalomail" => "STOREID", "magento"=>  "2"),
            "_1469026825907_907" => array("squalomail" => "STORENAME", "magento" => "3"),
            "_1469027411717_717" => array("squalomail" => "PREFIX","magento" => "4"),
            "_1469027418285_285" => array("squalomail" => "FNAME", "magento" => "5")
        );
            ...
            (
                ...
                (
                    ...
                    (
                        ...
                        (
                            "rel" => "self",
                            "href" => "https://api.staging.squalomail.com/mc/v3/lists/b514eebd1a/merge-fields/3",
                            "method" => "GET",
                            "targetSchema" => "https://api.staging.squalomail.com/mc/v3"
                        ...
                        (
                            "rel" => "parent",
                            "href" => "https://api.staging.squalomail.com/mc/v3/lists/b514eebd1a/merge-fields",
                            "method" => "GET",
                            "targetSchema" => "https://api.staging.squalomail.com/mc/v3"
                                . "/schema/3.0/Definitions/Lists/MergeFields/CollectionResponse.json",
                            "schema" => "https://api.staging.squalomail.com/mc/v3"
        ...
        $ebizmartsSqualomailMock = $this->getMockBuilder(Ebizmarts_SqualoMail::class)
            ->disableOriginalConstructor()->setMethods(array('getLists'))
            ->getMock();

        $squalomailListsMock = $this->getMockBuilder(SqualoMail_Lists::class)
            ->disableOriginalConstructor()->setMethods(array('getMergeFields'))
            ->getMock();

        $squalomailListsMergeFieldsMock = $this->getMockBuilder(SqualoMail_ListsMergeFields::class)
        ...
        $helperDataMock->expects($this->once())->method('getApi')->with($scopeId, $scope)
            ->willReturn($ebizmartsSqualomailMock);

        $ebizmartsSqualomailMock->expects($this->once())->method('getLists')
            ->willReturn($squalomailListsMock);

        $squalomailListsMock->expects($this->once())->method('getMergeFields')
            ->willReturn($squalomailListsMergeFieldsMock);

        $squalomailListsMergeFieldsMock->expects($this->once())->method('getAll')
    ...
    public function testGetCustomMergeFields()
    {
        ...
        $mapFieldsUnserialized = array(
            "_1468601283719_719", array("squalomail", "WEBSITE", "magento", "1"), "_1468609069544_544",
            array("squalomail", "STOREID", "magento", "2"), "_1469026825907_907",
            array("squalomail", "STORENAME", "magento", "3"),
            "_1469027411717_717", array("squalomail", "PREFIX", "magento", "4"), "_1469027418285_285",
            array("squalomail", "FNAME", "magento", "5")
        );
    ...
    public function testmodifyCounterDataSentToSqualomail()
    {
        $index = 0;
        $statusChanged = "SENT";
        ...
        $helperDataMock = $this->getMockBuilder(Ebizmarts_SqualoMail_Helper_Data::class)
            ->disableOriginalConstructor()
            ->setMethods(array('getCountersDataSentToSqualomail', 'setCountersDataSentToSqualomail'))
            ->getMock();

        $helperDataMock->expects($this->once())->method('getCountersDataSentToSqualomail');
        $helperDataMock->expects($this->once())->method('setCountersDataSentToSqualomail')
            ->with($index, $statusChanged, 1);

        $helperDataMock->modifyCounterDataSentToSqualomail($index);
    }

    public function testmodifyCounterDataNotSentToSqualomail()
    {
        $index = 0;
        ...
        $helperDataMock->modifyCounterDataSentToSqualomail($index, true);
    }
