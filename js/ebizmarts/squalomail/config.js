...
function loadStores()
{
    var apiKey = $('squalomail_general_apikey').value;
    $("squalomail_general_storeid").select('option').each(
        function (i) {
            i.remove();
        }
    );
            ...
            onComplete: function (transport) {
                ...
                if (json.length) {
                    for (var i = 0; i < json.length; i++) {
                        ...
                        $("squalomail_general_storeid").options.add(option);
                    }
                }
                loadList();
            }
...
function validateAPIkey($apiKeyInput, jsonInfo)
{
    ...
    let comment = $$('#row_squalomail_general_apikey p.note')[0];
    ...
    $('squalomail_general_account_details').insert("<li>" + jsonInfo[0].label + "</li>");
}

function loadInfo()
{
    const syncLabelKey = 11;
    var storeId = $('squalomail_general_storeid').value;

    let $apiKeyInput = $('squalomail_general_apikey');
    var apiKey = $apiKeyInput.value;
    $('squalomail_general_account_details').select('li').each(
        function (i) {
            i.remove();
        }
    );
    new Ajax.Request(
        MGETINFOURL, {
            method: 'get',
            parameters: {api_key: apiKey, squalomail_store_id: storeId},
                ...
                if (jsonInfo.length) {
                    ...
                    for (var i = 0; i < jsonInfo.length; i++) {
                        if (jsonInfo[i].value === syncLabelKey) {
                            $('squalomail_general_account_details').insert(jsonInfo[i].label);
                        } else {
                            $('squalomail_general_account_details').insert("<li>" + jsonInfo[i].label + "</li>");
                        }
                    }
                }
            }
...
function loadList()
{
    var storeId = $('squalomail_general_storeid').value;
    var apiKey = $('squalomail_general_apikey').value;
    var listId = $("squalomail_general_list");
                ...
                if (json.length) {
                    for (var i = 0; i < json.length; i++) {
                        if (json[i].value === listSelected || (json.length === 1)) {
                            var inheritField = $('squalomail_general_list_inherit');
                            var option = null;

                            if (inheritField && inheritField.checked === true) {
                                inheritField.checked = false;
                                $("squalomail_general_list").disabled = false;
                            }
...
{
    var listOptions = $('squalomail_general_list');
    var index = listOptions.selectedIndex;
    var listIdOptions = listOptions.options[index];

    $("squalomail_general_interest_categories").select('option').each(
        function (i) {
            if (i.selected && firstTime) {
                ...
                onComplete: function (transport) {
                    ...
                    if (json.length) {
                        for (var i = 0; i < json.length; i++) {
                            ...
                            $("squalomail_general_interest_categories").options.add(option);
                        }
                    }
                }
            }
...
function initAdmin()
{
    $('squalomail_general_apikey').onchange = changeApikey;
    $('squalomail_general_storeid').onchange = loadList;
    $('squalomail_general_list').onchange = loadInterest;
}
